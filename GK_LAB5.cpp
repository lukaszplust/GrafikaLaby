...
//-------------Atrybuty wierzcholkow------------------------------------------
	...
	GLfloat ver_triangle[1200] = {	//wspolrzedne wierzcholkow trojkata
		 0.0f,  1.0f, 0.0,
		 1.0f,  0.0f, 0.0,
		-1.0f,  0.0f, 0.0
	};

	GLfloat col_triangle[1200] = {	//kolory wierzcholkow trojkata
		1.0f, 0.5f, 0.0f,
		0.5f, 1.0f, 0.5f,
		0.0f, 0.5f, 1.0f
	};
...
void walec()
{
	...
	for(int i=0;i<(n*2);++i)
	{
	ver_triangle[6*i]=0.5*sin(i*a);
	ver_triangle[6*i+1]=0;
	ver_triangle[6*i+2]=0.5*cos(i*a);

	ver_triangle[6*i+3]=0.5*sin(i*a);
	ver_triangle[6*i+4]=2;
	ver_triangle[6*i+5]=0.5*cos(i*a);

	col_triangle[6*i]=0.5;
	col_triangle[6*i+1]=0.5;
	col_triangle[6*i+2]=0;

	col_triangle[6*i+3]=0.7;
	col_triangle[6*i+4]=0.1;
	col_triangle[6*i+5]=1;
...
int initGL(void)
{
   walec();
	...
	viewMatrix = glm::lookAt(glm::vec3(0.0f, 6.0f, 5.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f)); 
...
int drawGLScene(int counter)
{
   ...
   { glm::mat4 translationMatrix = glm::translate(glm::mat4(), glm::vec3(0.0f, 0.0f, 0.0f));  		//macierz przesuniecia o zadany wektor
    ...
    glm::mat4 rot1 = glm::rotate(glm::mat4(), glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));

    glm::mat4 scaling = glm::scale(glm::mat4(),glm::vec3((1.0f-i*0.15), (1.0f-i*0.15), (1.0f-i*0.15)));
    glm::mat4 translate = glm::translate(glm::mat4(),glm::vec3(0.0f,0.0f,-i*1.0f*0.5-1.2));
    glm::mat4 transformMatrix = projectionMatrix * viewMatrix * translationMatrix * rotationMatrix * rot1*translate*scaling; //wygenerowanie macierzy uwzgledniajacej wszystkie transformacje
    ...
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 250); //rysowanie trojkata
if(i==1)
    {glBindVertexArray(vao[1]);
